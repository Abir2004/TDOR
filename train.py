...
if __name__ == "__main__":
    ...
    tr_dl = DataLoader(tr_set, batch_size=train_batch_size, shuffle=True, num_workers=8)

    val_dl = DataLoader(val_set, batch_size=64, shuffle=False, num_workers=8)
            ...
            if i % eval_every == 0:
                ...
                with torch.no_grad():
                    ...
                    for k, data_val in enumerate(val_dl):
                        loss, policy, traj, ogms_rce, ogms_ce, min_ade, count = net(
                            data_val, temp=temp, type=type, device=device
                        )
                ...
                print(
                    ...
                    format(temp, "0.5f"),
                    "| val",
                    format(agg_val_loss / val_batch_count, "0.3f"),
                    "| ade",
                    format(l_batch_loss / val_batch_count, "0.3f"),
                    "| policy",
                    format(policy_loss / val_batch_count, "0.3f"),
                    "| traj",
                    format(traj_loss / val_batch_count, "0.3f"),
                    "|ce",
                    format((traj_loss + policy_loss) / val_batch_count, "0.3f"),
                    "| ogms_rce",
                    format(ogms_rce_loss / val_batch_count, "0.3f"),
                    "| ogms_ce",
                    format(ogms_ce_loss / val_batch_count, "0.3f"),
                    "| Min epoch",
                    min_epoch,
                    "| Min val loss",
                    format(min_val_loss, "0.3f"),
                    "| T(s):",
                    int(time.time() - st_time),
                )
